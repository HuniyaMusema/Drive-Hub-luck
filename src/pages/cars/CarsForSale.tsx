  ...
  return (
    <PageShell>
      <div className="container mx-auto px-4 lg:px-8">
        <div className="mb-8">
          <p className="text-accent font-semibold text-sm uppercase tracking-widest mb-2">Inventory</p>
          <h1 className="text-3xl lg:text-4xl font-bold text-foreground">Cars for Sale</h1>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 mb-8">
          ...
          <div className="flex border rounded-lg overflow-hidden ml-auto">
            <button
              ...
              className={`p-2 ${view === "grid" ? "bg-primary text-primary-foreground" : "bg-card text-muted-foreground hover:text-foreground"} transition-colors`}
            >
        ...
        {showFilters && (
          <div className="mb-8 p-4 bg-card rounded-xl shadow-sm animate-fade-in-up">
            <p className="text-sm font-semibold text-foreground mb-3">Brand</p>
                  ...
                  <div className="relative aspect-[4/3] overflow-hidden">
                    ...
                    {!car.available && (
                      <div className="absolute inset-0 bg-foreground/50 flex items-center justify-center">
                        <span className="bg-destructive text-destructive-foreground text-xs font-bold px-3 py-1 rounded-full">Sold</span>
                      </div>
                    )}
                    {car.available && (
                      <span className="absolute top-3 left-3 bg-primary text-primary-foreground text-xs font-bold px-3 py-1 rounded-full">Available</span>
                    )}
                  </div>
                  <div className="p-5">
              ...
              {cars.map((car, i) => (
                ...
                >
                  <div className="relative sm:w-64 aspect-[4/3] sm:aspect-auto overflow-hidden shrink-0">
                    <img src={car.image} alt={car.name} className="w-full h-full object-cover group-hover:scale-[1.03] transition-transform duration-700" />
                    {!car.available && (
                      <span className="absolute top-3 left-3 bg-destructive text-destructive-foreground text-xs font-bold px-3 py-1 rounded-full">Sold</span>
                    )}
                  </div>
