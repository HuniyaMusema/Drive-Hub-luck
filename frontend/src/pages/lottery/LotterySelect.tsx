  ...
  const toggle = (n: number) => {
    ...
    setSelected((prev) => {
      ...
      if (next.has(n)) next.delete(n);
        ...
        next.add(n);
        // On mobile (card is below the grid), scroll the summary card into view
      ...
      <div className="container mx-auto px-4 lg:px-8 pb-32">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-4 lg:sticky lg:top-40 h-fit">
            <div ref={summaryCardRef} className="bg-card rounded-3xl shadow-2xl shadow-primary/5 p-7 border border-border/60 relative overflow-hidden group/card animate-slide-in-right">
              ...
              <div className="relative z-10">
                ...
                <div className="flex items-center gap-3 mb-6">
                  <div className="w-11 h-11 rounded-2xl bg-primary/10 flex items-center justify-center border border-primary/20">
                    <ShoppingCart className="h-5 w-5 text-primary" strokeWidth={2.5} />
                  </div>
                  <div>
                ...
                {selected.size === 0 ? (
                  <div className="py-8 text-center border-2 border-dashed border-border/40 rounded-2xl flex flex-col items-center gap-2">
                    <Hash className="h-7 w-7 text-muted-foreground opacity-10" strokeWidth={1} />
                    <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40">{t("noNumbersSelected")}</p>
                  </div>
                ) : (
                  <div className="space-y-4 animate-fade-in">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {[...selected].sort((a, b) => a - b).map((n) => (
                        <div key={n} className="bg-primary px-4 py-2 rounded-xl shadow-md shadow-primary/20 flex items-center gap-2 cursor-default">
                          <span className="text-primary-foreground font-black tabular-nums text-base">{n.toString().padStart(2, '0')}</span>
                    ...
                    <div className="space-y-2 pt-3 border-t border-border/60">
                      ...
                      <div className="flex justify-between items-end pt-3 border-t border-border/60">
                        <div>
                          <span className="text-[9px] font-black uppercase tracking-[0.3em] text-primary">{t("total")}</span>
                          <h3 className="text-3xl font-black text-foreground tabular-nums tracking-tighter leading-none">
                            {(selected.size * ticketPrice).toLocaleString()}
                          </h3>
                        </div>
                        <span className="text-xs font-black text-primary uppercase pb-1">{currency}</span>
                      </div>
                    </div>
                  </div>
                )}
                ...
                <Button
                  className="w-full mt-6 rounded-2xl h-14 text-base font-black shadow-xl shadow-primary/30 transition-all hover:scale-[1.02] active:scale-95 group/submit"
                ...
                >
                  {participateMutation.isPending ? (
                    ...
                    <>
                      {t("confirmSelection")}
                      <div className="ml-2 w-6 h-6 rounded-lg bg-white/20 flex items-center justify-center group-hover/submit:translate-x-1 transition-transform">
