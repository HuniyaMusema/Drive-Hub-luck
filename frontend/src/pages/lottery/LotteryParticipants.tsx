...
const statusConfig: Record<string, { label: string; className: string }> = {
  confirmed: { label: "Confirmed", className: "bg-[#4CBFBF]/10 text-[#4CBFBF] border-[#4CBFBF]/20" },
  pending:   { label: "Pending",   className: "bg-amber-400/10 text-[#f5b027] border-amber-400/20" },
  ...
  return (
    <AdminLayout>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-10">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mt-1.5 line-clamp-1">
              {t("lpActiveDraw")} <span className="text-[#4CBFBF] font-black tracking-widest">#{lotteryData?.lottery?.id.split('-')[0].toUpperCase() ?? "None"}</span>
            </p>
          </div>
        </div>
        <div className="relative w-full sm:w-80">
          ...
          <Input
            className="pl-12 rounded-2xl bg-white border-slate-200 focus-visible:ring-[#4CBFBF]/20 h-12 font-black text-slate-900 placeholder:text-slate-300"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-10">
        {[
          { label: t("lpTotalParticipants"), value: participants.length, icon: Users, color: "text-slate-900", bg: "bg-white", border: "border-slate-200 shadow-xl shadow-slate-100" },
          { label: t("lpConfirmedTickets"),  value: confirmedCount,      icon: CheckCircle2, color: "text-[#4CBFBF]", bg: "bg-slate-50", border: "border-slate-200 shadow-xl shadow-slate-100" },
          { label: t("lpPendingReview"),     value: pendingCount,        icon: Clock,  color: "text-[#f5b027]", bg: "bg-slate-50", border: "border-slate-200 shadow-xl shadow-slate-100" },
        ].map(({ label, value, icon: Icon, color, bg, border }) => (
          <div key={label} className={`rounded-[2rem] p-8 border ${border} ${bg} hover:translate-y-[-4px] transition-all relative overflow-hidden group`}>
            ...
            <p className="text-[10px] text-slate-500 uppercase font-black tracking-[0.2em] mb-4">{label}</p>
            <div className="flex items-end justify-between">
              <p className={`text-4xl font-black tabular-nums tracking-tighter ${color}`}>{value}</p>
              <Icon className={`h-10 w-10 ${color} opacity-20 group-hover:scale-110 transition-transform group-hover:opacity-40`} />
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white rounded-[2.5rem] shadow-xl border border-slate-200 overflow-hidden shadow-slate-100 mb-10">
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 gap-6 text-center">
              <div className="w-20 h-20 rounded-3xl bg-slate-50 flex items-center justify-center border border-slate-200 shadow-inner">
                <Users className="h-10 w-10 text-slate-300" strokeWidth={1} />
              </div>
              <div>
                ...
                <p className="text-slate-500 text-[10px] uppercase font-black tracking-widest mt-2">{t("lpParticipantsAppearOnce")}</p>
              </div>
            </div>
          ) : (
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 text-left bg-slate-50/50">
                  ...
                  <th className="px-8 py-6 font-black text-slate-400 uppercase tracking-[0.2em] text-[9px]">{t("lpParticipant")}</th>
                  <th className="px-8 py-6 font-black text-slate-400 uppercase tracking-[0.2em] text-[9px]">{t("lpTicketNumbers")}</th>
                  <th className="px-8 py-6 font-black text-slate-400 uppercase tracking-[0.2em] text-[9px] text-center">{t("lpCount")}</th>
                  <th className="px-8 py-6 font-black text-slate-400 uppercase tracking-[0.2em] text-[9px] text-right">{t("lpJoined")}</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map((p, i) => {
                  ...
                  return (
                    <tr key={p.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-colors group">
                      ...
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-4">
                          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-xs font-black shrink-0 border transition-all ${hasConfirmed ? 'bg-[#4CBFBF]/10 text-[#4CBFBF] border-[#4CBFBF]/20' : 'bg-amber-400/10 text-[#f5b027] border-amber-400/20'}`}>
                            {p.name.charAt(0).toUpperCase()}
                          </div>
                          <p className="font-black text-slate-900 tracking-tighter uppercase text-[11px] group-hover:text-[#4CBFBF] transition-colors">{p.name}</p>
                        </div>
                      </td>
                      <td className="px-8 py-6">
                      ...
                      <td className="px-8 py-6 tubular-nums font-black text-center text-2xl text-[#4CBFBF] tracking-tighter">{p.numbers.length}</td>
                      <td className="px-8 py-6 text-slate-400 tabular-nums text-right text-[10px] font-black uppercase tracking-[0.2em]">
        ...
        {filtered.length > 0 && (
          <div className="px-8 py-6 border-t border-slate-100 bg-slate-50/50 text-[10px] font-black uppercase tracking-widest text-slate-400 flex justify-between">
            ...
            {search && (
              <button onClick={() => setSearch("")} className="text-[#4CBFBF] hover:text-[#3fb0b0] font-black tracking-widest">{t("lpClearSearch")}</button>
            )}
          </div>
        )}
      </div>
